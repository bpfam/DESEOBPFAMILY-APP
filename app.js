    ...
    switch(section){
        ...
        case "info":
            content.innerHTML = `
                ...
                <p>Solo per maggiorenni. Disponibile dove consentito dalla legge locale.</p>
            `;
            break;
...
function showReviews(){
    ...
    if(reviews.length === 0){
        content.innerHTML = `<h2>â­ Recensioni</h2><p>Nessuna recensione disponibile.</p>`;
        return;
    }
        ...
        ${reviews.map(review => `
            <div class="product-card">
                ${review.type === "video"
                    ...
                    : `<img src="${review.media}" class="product-img" alt="${review.title || "Recensione"}" loading="lazy">`
                }
                <h3>${review.title || "Recensione"}</h3>
                <p>${"â­".repeat(Number(review.rating) || 5)}</p>
                <p>${review.text || ""}</p>
            </div>
        `).join("")}
    `;
...
function showVetrina(category){
    ...
    if(products.length === 0){
        content.innerHTML = `
            ...
            <button class="contact-btn" onclick="retryProducts()">Riprova</button>
        `;
        return;
    }
    ...
    const categoryHTML = categories.map(cat => `
        <button class="category-pill" onclick="showVetrina('${String(cat).replace(/'/g, "\\'")}')">${cat}</button>
    `).join("");
    ...
    const productHTML = filteredProducts.map(product => {
        ...
        return `
            <div class="product-card">
                ...
                ${product.quality ? `
                    <p><strong>ð QualitÃ :</strong> ${product.quality}</p>
                ` : ""}

                ${product.origin ? `
                    <p><strong>ð Origine:</strong> ${product.origin}</p>
                ` : ""}

                <p><strong>ð Categoria:</strong> ${product.category || "Altro"}</p>
                ...
                <div class="product-meta">
                    <span>${product.status || "Disponibile"}</span>
                    <span id="price-${realIndex}">â¬${firstVariant.price}</span>
                </div>

                <button class="contact-btn" onclick="addToCart(${realIndex})">
                    ð Aggiungi al carrello
                </button>
    ...
    content.innerHTML = `
        ...
        ${productHTML || "<p>Nessun prodotto presente in questa categoria.</p>"}
    `;
}
...
function updatePrice(index, variantIndex){
    ...
    const variants = getVariants(product);
    const selectedVariant = variants[Number(variantIndex)];
    const priceBox = document.getElementById(`price-${index}`);

    if(priceBox && selectedVariant){
        priceBox.textContent = `â¬${selectedVariant.price}`;
    }
}
...
function addToCart(index){
    ...
    const selectedIndex = select ? Number(select.value) : 0;
...
function updateCartCount(){
    ...
    const sectionCartCount = document.getElementById("sectionCartCount");

    if(homeCartCount) homeCartCount.textContent = totalItems;
    if(sectionCartCount) sectionCartCount.textContent = totalItems;
}
...
function showCart(){
    ...
    if(cart.length === 0){
        content.innerHTML = `
            ...
            <button class="contact-btn" onclick="openSection('vetrina')">Apri la Vetrina</button>
        `;
        return;
    }
    ...
    content.innerHTML = `
        ...
        <button class="remove-btn" onclick="clearCart()" style="width:100%; margin-top:15px;">
            Svuota carrello
        </button>

        <div class="product-card">
            <h3>Totale</h3>
            <p class="total-price">â¬${total}</p>
        </div>

        <label class="format-label">Metodo ordine</label>
        <select class="format-select" id="orderMethod" onchange="showOrderFields()">
            <option value="Meet Up">ð Meet Up</option>
            <option value="Delivery">ð Delivery</option>
            <option value="Spedizione">ð¦ Spedizione</option>
        </select>
        ...
        <select class="format-select" id="paymentMethod">
            <option value="Contanti">ðµ Contanti</option>
            <option value="Crypto">â¿ Crypto</option>
            <option value="Da concordare">ð¬ Da concordare</option>
        </select>
...
function showOrderFields(){
    ...
    if(method === "Spedizione"){
        box.innerHTML = `
            ...
            <label class="format-label">CittÃ </label>
            <input class="format-select" id="city" placeholder="CittÃ ">
            ...
            <input class="format-select" id="email" type="email" placeholder="Email">

            <label class="format-label">Telefono</label>
            <input class="format-select" id="phone" type="tel" placeholder="Telefono">
    ...
    if(method === "Delivery"){
        box.innerHTML = `
            ...
            <input class="format-select" id="date" type="date">
    ...
    if(method === "Meet Up"){
        box.innerHTML = `
            ...
            <input class="format-select" id="time" type="time">
...
function buildOrderMessage(){
    let message = "Ciao, vorrei informazioni per questo ordine:\n\n";

    cart.forEach((item, index) => {
        message += `${index + 1}. ${item.name}\n`;
        message += `Formato: ${item.size}\n`;
        message += `QuantitÃ : ${item.quantity}\n`;
        message += `Totale articolo: â¬${item.price * item.quantity}\n\n`;
    });
...
function sendOrder(){
    ...
    if(method === "whatsapp"){
        const separator = contacts.whatsapp.includes("?") ? "&" : "?";
        window.open(`${contacts.whatsapp}${separator}text=${encodedMessage}`, "_blank");
    }
...
function showContatti(){
    ...
    content.innerHTML = `
        <h2>ð Contatti ufficiali</h2>
        <p>Scegli dove contattarci.</p>

        <div class="contact-list">
            <a class="contact-link" href="${contacts.telegram}" target="_blank" rel="noopener noreferrer">ð¬ Telegram</a>
            <a class="contact-link" href="${contacts.instagram}" target="_blank" rel="noopener noreferrer">ð¸ Instagram</a>
            <a class="contact-link" href="${contacts.whatsapp}" target="_blank" rel="noopener noreferrer">ð¢ WhatsApp</a>
            <a class="contact-link" href="${contacts.signal}" target="_blank" rel="noopener noreferrer">ð Signal</a>
        </div>
    `;
}
    ...
    document.getElementById("content").innerHTML = `
        ...
        ${results.map(product => `
            <div class="product-card">
                ...
                ${product.quality ? `<p><strong>ð QualitÃ :</strong> ${product.quality}</p>` : ""}
                ${product.origin ? `<p><strong>ð Origine:</strong> ${product.origin}</p>` : ""}
